...
const Foam::word
Foam::radiation::radiativeIntensityRayBand::intensityPrefix("ILambda");


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::radiation::radiativeIntensityRayBand::radiativeIntensityRayBand
(
    const fvDOMBand& dom,
    const fvMesh& mesh,
    const scalar phi,
...
:
    dom_(dom),
    mesh_(mesh),
    absorptionEmissionModel_(absorptionEmission),
...
// WSGGM (added by Ivan Sikic 26/03/2015)
    ...
    (
        ...
        mesh_,
        dimensionedScalar("kI", dimMass/pow3(dimTime)/dimLength, 0.0)
    ),
    Qr_
    (
        IOobject
...
Foam::scalar Foam::radiation::radiativeIntensityRayBand::correct()
{
    ...
    forAll(ILambda_, lambdaI)
    {
        ...
        const volScalarField& wsggmWeightingCoeff= dom_.ggCoeffLambda(lambdaI); // WSGGM (added by Ivan Sikic 10/10/2014)
...
void Foam::radiation::radiativeIntensityRayBand::addIntensity()
{
    I_ = dimensionedScalar("zero", dimMass/pow3(dimTime), 0.0);
    kI_ = dimensionedScalar("zero", dimMass/pow3(dimTime)/dimLength, 0.0);
