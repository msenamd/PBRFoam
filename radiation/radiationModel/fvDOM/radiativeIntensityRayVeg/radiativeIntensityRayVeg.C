...
#include "radiativeIntensityRayVeg.H"
#include "fvm.H"
#include "fvDOMVeg.H"
#include "constants.H"
...
const Foam::word Foam::radiation::radiativeIntensityRayVeg::intensityPrefix
(
    "ILambda"
);
...
Foam::radiation::radiativeIntensityRayVeg::radiativeIntensityRayVeg
(
    const fvDOMVeg& dom,
    const fvMesh& mesh,
    const scalar phi,
...
Foam::scalar Foam::radiation::radiativeIntensityRayVeg::correct()
{
    ...
    forAll(ILambda_, lambdaI)
    {
        ...
        ==
            ...
            omega_*(
                1.0/constant::mathematical::pi
               *(
                    absorptionEmission_.aCont(lambdaI)
                   *blackBody_.bLambda(lambdaI)                 //kappa_gas*sigma*T^4

                  + absorptionEmission_.ECont(lambdaI)/4        //Prescribed radiation emission (here divided by 4*pi)
                )
               + absorptionEmission_.EDisp(lambdaI)             //Vegetation bed emission term
                )
        );
...
void Foam::radiation::radiativeIntensityRayVeg::addIntensity()
{
