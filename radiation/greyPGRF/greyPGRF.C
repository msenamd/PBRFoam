...
namespace Foam
{
    namespace radiation
    {
        defineTypeNameAndDebug(greyPGRF, 0);
...
Foam::radiation::greyPGRF::greyPGRF
(
...
Foam::radiation::greyPGRF::aCont(const label bandI) const
{
...
Foam::tmp<Foam::volScalarField>
Foam::radiation::greyPGRF::eCont(const label bandI) const
{
   return aCont(bandI);
}
...
Foam::radiation::greyPGRF::ECont(const label bandI) const
{
    ...
    forAll (dQ, celli)
    {
        if (dQ[celli] >= 1000)
        {
            E.ref()[celli] = EhrrCoeff_*dQ[celli];
        }
    }
